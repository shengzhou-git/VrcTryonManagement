/**
 * Next.js API 路由 - 上传准备（生成 presigned uploadUrl）
 * 服务器端代理，保护 API Key
 */
...
export async function POST(request: NextRequest) {
  try {
    // 注意：一定要在运行时读取 env（Amplify/Next 构建期可能会把模块顶层的 process.env 固化为空）
    ...
    const response = await fetch(`${AWS_API_URL}/upload/prepare`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: data.error || '上传准备失败' }, { status: response.status })
    }

    return NextResponse.json(data)
  } catch (error) {
    console.error('Upload prepare API error:', error)
